...
SponzaScene::SponzaScene(WolfEngine* wolfInstance, std::mutex* vulkanQueueLock)
{
	...
	m_forwardPass.reset(new ForwardPass(m_sponzaModel->getMesh(), sponzaImages, m_depthPass->getSemaphore(), m_depthPass.get()));
...
{
	std::vector<Wolf::CommandRecordBase*> passes(2);
